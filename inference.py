    ...
    with tf.Session(config=config) as sess:
        ...
        for i, a_img_name in enumerate(real_test_imgname_list):
            ...
            cv2.imwrite(inference_save_path + '/' + nake_name,
                        final_detections[:, :, ::-1])
            time_Array_once=end-start
            time_Array.append(time_Array_once)

            tools.view_bar('{} image cost {}s'.format(a_img_name, (end - start)), i + 1, len(real_test_imgname_list))
        np.savetxt(r'/raid/Wei/Codes/TestCode/Multi_Level_Multi_attentional_Net/MLMA_Net/tools/time_Array.csv',time_Array, delimiter=',')
...
def parse_args():
    ...
    parser.add_argument('--data_dir', dest='data_dir',
                        help='data path',
                        default='/raid/Wei/Codes/TestCode/Multi_Level_Multi_attentional_Net/MLMA_Net/tools/demos/', type=str)
    parser.add_argument('--save_dir', dest='save_dir',
                        help='demo imgs to save',
                        default='/raid/Wei/Codes/TestCode/Multi_Level_Multi_attentional_Net/MLMA_Net/tools/inference_results/', type=str)
