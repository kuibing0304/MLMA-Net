    ...
    def build_whole_detection_network(self, input_img_batch, gtboxes_batch):
        ...
        with tf.variable_scope('build_rpn',
            ...
            for level_name, p in zip(cfgs.LEVLES, P_list):
                ...
                #Multi attentional Module start    p include p2, p3...
                X_atten=slim.conv2d(
                    rpn_conv3x3, 256, [1, 1],
                    trainable=self.is_training, weights_initializer=cfgs.INITIALIZER, padding="SAME",
                    scope='1x1_conv_atten',
                    activation_fn=tf.nn.relu,
                    reuse=tf.AUTO_REUSE)              #512replaced by 256
                X_atten=slim.batch_norm(X_atten)
                X_atten=tf.sigmoid(0.5*X_atten)       # B is setup to 10
                X_atten=tf.math.multiply(X_atten,p)
                # Multi attentional Module end

                # obtain the scole of rpn classification and scole of rpn bbox
                # rpn_conv3x3 replaced by X_atten
                rpn_cls_score = slim.conv2d(X_atten, self.num_anchors_per_location*2, [1, 1], stride=1,
                                            trainable=self.is_training, weights_initializer=cfgs.INITIALIZER,
                                            activation_fn=None, padding="VALID",
                                            scope=scope_list[1],
                                            reuse=reuse_flag)
                rpn_box_pred = slim.conv2d(X_atten, self.num_anchors_per_location*4, [1, 1], stride=1,
